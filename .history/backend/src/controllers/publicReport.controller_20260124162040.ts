...
export const getPublicReport = async (req: Request, res: Response) => {
  try {
    ...
    const totalTests = publicLink.testRun.executions.length;
    const passed = publicLink.testRun.executions.filter(e => e.result === 'PASSED').length;
    const failed = publicLink.testRun.executions.filter(e => e.result === 'FAILED').length;
    const skipped = publicLink.testRun.executions.filter(e => e.result === 'SKIPPED').length;
    const blocked = publicLink.testRun.executions.filter(e => e.result === 'BLOCKED').length;

    res.json({
      report: {
        testRun: publicLink.testRun,
        stats: {
          total: totalTests,
          passed,
          failed,
