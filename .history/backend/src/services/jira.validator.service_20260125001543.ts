  ...
  private normalizeUrl(url: string): string {
    try {
      ...
      if (normalized.includes('continue=')) {
        ...
        // Extract the Jira URL from continue parameter
        const continueMatch = normalized.match(/continue=(https?:\/\/[^&\s]+)/);
        if (continueMatch) {
          normalized = continueMatch[1].split('/')[0] + '//' + continueMatch[1].split('/')[2];
        }
      }

      // Extract Jira domain if we have atlassian.net but not a full URL
      if (!normalized.startsWith('http') && normalized.includes('atlassian.net')) {
      ...
      console.log(`[Jira Validator] Normalized URL from "${this.rawUrl}" to "${normalized}"`);
