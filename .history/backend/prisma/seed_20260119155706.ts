...
async function main() {
  ...
  const testCase1 = await prisma.testCase.create({
    data: {
      ...
      description: 'Test case to verify user can login with valid username and password',
  ...
  const testCase2 = await prisma.testCase.create({
    data: {
      ...
      description: 'Test case to verify user cannot login with invalid password',
