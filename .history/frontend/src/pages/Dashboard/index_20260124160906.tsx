...
const Dashboard: React.FC = () => {
  ...
  const loadDashboardData = useCallback(async () => {
    try {
      ...
      setLoadingAI(true);
      try {
        const suggestionsRes = await aiService.getSuggestions(currentProject!.id, { status: 'PENDING' });
        setAiSuggestions((suggestionsRes.data?.data || []).slice(0, 5));
        
        const gapsRes = await aiService.getCoverageGaps(currentProject!.id);
        setCoverageGaps((gapsRes.data?.data || []).length);
      } catch (aiError) {
        console.log('AI features not yet available:', aiError);
        // Set mock data for demonstration
        setAiSuggestions([
          { id: '1', title: 'Add negative test case for login validation', confidence: 0.92, type: 'TEST_CASE' },
          { id: '2', title: 'Test password reset flow edge cases', confidence: 0.88, type: 'TEST_CASE' },
          { id: '3', title: 'Verify session timeout handling', confidence: 0.85, type: 'TEST_CASE' },
        ]);
        setCoverageGaps(3);
      } finally {
        setLoadingAI(false);
  ...
  return (
    <Box>
      ...
      <Alert 
        severity="success" 
        icon={<AutoAwesome />}
        sx={{ mb: 3, fontWeight: 500, background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', color: 'white', border: 'none' }}
        action={
          <Button color="inherit" size="small" sx={{ color: 'white', borderColor: 'white' }} variant="outlined">
            <Rocket sx={{ mr: 1 }} /> Explore AI Features
          </Button>
        }
      >
        <strong>🚀 New AI-Powered Features Available!</strong><br/>
        ✨ AI Test Generation &nbsp; | &nbsp; 🔍 Flaky Test Detection &nbsp; | &nbsp; 🥒 BDD/Gherkin Support &nbsp; | &nbsp; 🔧 CI/CD Integration &nbsp; | &nbsp; 🌿 Git-like Branching
      </Alert>
      ...
      {/* NEW: AI-Powered Widgets Section */}
      <Grid container spacing={3} sx={{ mb: 3 }}>
        {/* AI Test Suggestions Widget */}
          ...
          >
            <CardContent sx={{ position: 'relative', zIndex: 1 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
                ...
                <Chip 
                  label={`${aiSuggestions.length} New`} 
              ...
              {loadingAI ? (
                <Box sx={{ textAlign: 'center', py: 3 }}>
                  <LinearProgress sx={{ mb: 2, backgroundColor: 'rgba(255,255,255,0.3)' }} />
                  <Typography variant="body2">Analyzing your test coverage...</Typography>
                </Box>
              ) : aiSuggestions.length > 0 ? (
                <Box>
                  <Typography variant="body2" sx={{ mb: 2, opacity: 0.95 }}>
                    🤖 AI analyzed your project and found opportunities to improve test coverage:
                  </Typography>
                    ...
                    {aiSuggestions.map((suggestion: any) => (
                      ...
                      >
                        ...
                        <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
                          <Chip 
                            label={`${Math.round((suggestion.confidence || 0.85) * 100)}% Confidence`} 
                            size="small" 
                            sx={{ bgcolor: 'rgba(76, 175, 80, 0.3)', color: 'white', fontWeight: 600 }}
                          />
                          <Chip 
                            label={suggestion.type || 'TEST_CASE'} 
                            size="small" 
                  ...
                  >
                    Accept All Suggestions ({aiSuggestions.length})
                  </Button>
                </Box>
              ) : (
                <Alert severity="info" sx={{ bgcolor: 'rgba(255,255,255,0.15)', color: 'white', border: '1px solid rgba(255,255,255,0.3)' }}>
                  ✨ No pending suggestions. AI is monitoring your test coverage!
                </Alert>
              )}
            </CardContent>
          </Card>
        ...
        {/* Coverage Gaps & Quick Stats Widget */}
        <Grid item xs={12} md={6}>
          <Grid container spacing={2} sx={{ height: '100%' }}>
                ...
                <CardContent>
                  <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 1 }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                      ...
                      <Typography variant="h6" sx={{ fontWeight: 600 }}>Coverage Gaps</Typography>
                    </Box>
                    <Chip 
                      label={`${coverageGaps} Found`} 
                      sx={{ bgcolor: 'rgba(255,255,255,0.3)', color: 'white', fontWeight: 600 }}
                    />
                  </Box>
                  <Typography variant="body2" sx={{ opacity: 0.95 }}>
                    {coverageGaps > 0 
                      ? `🔍 ${coverageGaps} requirement(s) need test coverage. Click to generate AI tests.`
            ...
            <Grid item xs={6}>
              <Card sx={{ bgcolor: '#4caf50', color: 'white' }} elevation={3}>
                <CardContent sx={{ textAlign: 'center', py: 2 }}>
                  <Typography variant="h4" sx={{ fontWeight: 700 }}>🥒</Typography>
                  <Typography variant="body2" sx={{ fontWeight: 600 }}>BDD/Gherkin</Typography>
                  <Typography variant="caption">Behavior-Driven Tests</Typography>
                </CardContent>
              </Card>
            </Grid>

            <Grid item xs={6}>
              <Card sx={{ bgcolor: '#ff9800', color: 'white' }} elevation={3}>
                <CardContent sx={{ textAlign: 'center', py: 2 }}>
                  <Typography variant="h4" sx={{ fontWeight: 700 }}>🔧</Typography>
                  <Typography variant="body2" sx={{ fontWeight: 600 }}>CI/CD</Typography>
                  <Typography variant="caption">9 Integrations Ready</Typography>
                </CardContent>
              </Card>
            </Grid>
          </Grid>
