...
const Dashboard: React.FC = () => {
  ...
  return (
    <Box>
            ...
            <CardContent>
              <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  ...
                  <Badge badgeContent={aiSuggestions.length} color="error" sx={{ ml: 1 }} />
                </Box>
                <IconButton onClick={handleRefreshAI} size="small" sx={{ color: 'white' }}>
                  <Refresh />
                </IconButton>
              </Box>
              ...
              {loadingAI ? (
                ...
                <Box>
                  <Typography variant="body2" sx={{ mb: 2, opacity: 0.9 }}>
                    AI has analyzed your requirements and generated {aiSuggestions.length} test suggestions
                  </Typography>
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                    {aiSuggestions.slice(0, 3).map((suggestion: any) => (
                      <Paper key={suggestion.id} sx={{ p: 1.5, bgcolor: 'rgba(255,255,255,0.15)' }}>
                        <Typography variant="body2" sx={{ fontWeight: 600, mb: 0.5 }}>
                          {suggestion.title || suggestion.description?.substring(0, 50) + '...'}
                        </Typography>
                        <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
                          <Chip label={`${Math.round((suggestion.confidence || 0.8) * 100)}% confidence`} size="small" sx={{ bgcolor: 'rgba(255,255,255,0.2)', color: 'white' }} />
                          <Chip label={suggestion.type || 'TEST'} size="small" sx={{ bgcolor: 'rgba(255,255,255,0.2)', color: 'white' }} />
                        </Box>
                      </Paper>
                    ))}
                  </Box>
                  <Button 
                    variant="outlined" 
                    fullWidth 
                    sx={{ mt: 2, color: 'white', borderColor: 'white', '&:hover': { borderColor: 'white', bgcolor: 'rgba(255,255,255,0.1)' } }}
                  >
                ...
                <Alert severity="info" sx={{ bgcolor: 'rgba(255,255,255,0.15)', color: 'white' }}>
                  No pending AI suggestions. Create requirements to get AI-powered test recommendations.
                </Alert>
              )}
            </CardContent>
          </Card>
        ...
        <Grid item xs={12} md={6}>
          <Card elevation={3} sx={{ height: '100%', background: 'linear-gradient(135deg, #f093fb 0%, #f5576c 100%)', color: 'white' }}>
            <CardContent>
              <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  <BugReportOutlined sx={{ fontSize: 32 }} />
                  <Typography variant="h6" sx={{ fontWeight: 600 }}>
                    Coverage Gaps
                  </Typography>
                  <Badge badgeContent={coverageGaps.length} color="error" sx={{ ml: 1 }} />
                </Box>
                <MuiTooltip title="Requirements without test coverage">
              ...
              {loadingAI ? (
                ...
                <Box>
                  <Typography variant="body2" sx={{ mb: 2, opacity: 0.9 }}>
                    {coverageGaps.length} requirement(s) need test coverage
                  </Typography>
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                    {coverageGaps.slice(0, 3).map((gap: any, idx: number) => (
                      <Paper key={idx} sx={{ p: 1.5, bgcolor: 'rgba(255,255,255,0.15)' }}>
                        <Typography variant="body2" sx={{ fontWeight: 600, mb: 0.5 }}>
                          {gap.requirementTitle || gap.title}
                        </Typography>
                        <Chip 
                          label={gap.priority || 'MEDIUM'} 
                          size="small" 
                          color={gap.priority === 'HIGH' || gap.priority === 'CRITICAL' ? 'error' : 'warning'}
                ...
                <Alert severity="success" sx={{ bgcolor: 'rgba(255,255,255,0.15)', color: 'white' }}>
      ...
      >
        <strong>🚀 New AI-Powered Features Available!</strong> AI test generation, flaky test detection, BDD/Gherkin support, CI/CD integration, and Git-like branching now enabled.
      </Alert>
