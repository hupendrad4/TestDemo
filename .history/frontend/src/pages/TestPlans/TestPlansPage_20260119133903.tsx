...
const TestPlansPage: React.FC = () => {
  ...
  const wizardSteps = ['Basic Info', 'Select Test Cases', 'Assign Platforms', 'Set Milestone', 'Review'];
  ...
  const loadData = async () => {
    ...
    setLoading(true);
    try {
      const [plans, cases, platformsData, milestonesData, buildsData] = await Promise.all([
  ...
  const handleCreatePlan = async () => {
    ...
    try {
      const plan = await testPlanService.createTestPlan({
        name: planName,
      ...
      if (selectedTestCases.length > 0) {
        await testPlanService.addTestCases(plan.id, selectedTestCases);
      }
