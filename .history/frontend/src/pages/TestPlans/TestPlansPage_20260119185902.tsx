...
const TestPlansPage: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const reqsArray = Array.isArray(reqs) ? reqs : (reqs?.data || []);
      const casesArray = Array.isArray(cases) ? cases : (cases?.data || []);
      console.log('Loaded requirements:', reqsArray.length);
      console.log('Loaded test cases:', casesArray.length);
      setAvailableRequirements(reqsArray);
      setAvailableTestCases(casesArray);
            ...
            <List>
              {availableTestCases.map(testCase => (
                <ListItem key={testCase.id} button onClick={() => toggleTestCase(testCase.id)}>
                  ...
                  />
                  <ListItemText
                    primary={testCase.title || testCase.name}
      ...
      case 5:
        return (
          <Box>
            ...
            <Typography variant="body2" sx={{ mt: 1 }}>
              <strong>Requirements:</strong> {selectedRequirements.length} selected
            </Typography>
            <Typography variant="body2" sx={{ mt: 1 }}>
