...
const TestSuiteExplorer: React.FC = () => {
  ...
  const [watchedCases, setWatchedCases] = useState<Set<string>>(new Set());
  ...
  const [newCaseRequirementIds, setNewCaseRequirementIds] = useState<string[]>([]);
  const [newCasePreconditions, setNewCasePreconditions] = useState('');
  const [newCaseExpectedBehavior, setNewCaseExpectedBehavior] = useState('');
  const [newCaseActualBehavior, setNewCaseActualBehavior] = useState('');
  ...
  const loadWatchlist = async () => {
    try {
      const watchlist = await watchlistService.getWatchlist('TEST_CASE');
      const watchedIds = new Set(watchlist.map(w => w.entityId));
      setWatchedCases(watchedIds);
    } catch (error) {
  ...
  const loadRequirements = async () => {
    if (!currentProject?.id) return;
    try {
      const response = await requirementService.getRequirements(currentProject.id);
  ...
  const toggleWatchlist = async (caseId: string) => {
    try {
      if (watchedCases.has(caseId)) {
        const watchlist = await watchlistService.getWatchlist('TEST_CASE');
        const entry = watchlist.find(w => w.entityId === caseId);
        if (entry) {
          await watchlistService.removeFromWatchlist(entry.id);
        }
      } else {
        await watchlistService.addToWatchlist('TEST_CASE', caseId);
      }
      loadWatchlist();
    } catch (error) {
  ...
  const handleLoadView = (filterConfig: any) => {
    ...
    if (filterConfig.suiteId) {
      const suite = suites.find(s => s.id === filterConfig.suiteId);
  ...
  const handleCreateTestCase = async () => {
    if (!newCaseTitle.trim() || !currentProject?.id) return;
    try {
      const createdCase = await testCaseService.createTestCase(currentProject.id, {
        title: newCaseTitle,
        ...
        expectedBehavior: newCaseExpectedBehavior,
        actualBehavior: newCaseActualBehavior,
      });
      
      // Link requirements if any selected
      if (newCaseRequirementIds.length > 0 && createdCase.id) {
        await Promise.all(
          newCaseRequirementIds.map(reqId =>
            requirementService.linkTestCase(reqId, createdCase.id)
          )
        );
      }
      ...
      setNewCaseRequirementIds([]);
      setNewCasePreconditions('');
      setNewCaseExpectedBehavior('');
      setNewCaseActualBehavior('');
      loadData();
    } catch (error) {
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} md={3}>
          <Paper sx={{ p: 2, height: 'calc(100vh - 220px)', overflow: 'auto' }}>
            ...
            <List disablePadding>{suites && renderSuiteTree(suites)}</List>
          </Paper>
        </Grid>
                  ...
                  <TableHead>
                    <TableRow>
                      <TableCell padding="checkbox">
                        <Checkbox
                          indeterminate={selectedCases.length > 0 && selectedCases.length < (testCases?.length || 0)}
                          checked={(testCases?.length || 0) > 0 && selectedCases.length === (testCases?.length || 0)}
                      ...
                      testCases.map((testCase) => (
                        <TableRow key={testCase.id} hover>
                          ...
                          <TableCell>
                            <IconButton
                              size="small"
                              onClick={() => toggleWatchlist(testCase.id)}
                              color={watchedCases.has(testCase.id) ? 'primary' : 'default'}
                            >
                              {watchedCases.has(testCase.id) ? <Star /> : <StarBorder />}
                            </IconButton>
                          </TableCell>
      ...
      <Dialog open={createCaseOpen} onClose={() => setCreateCaseOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          ...
          <TextField
            ...
            multiline
            rows={2}
            value={newCaseExpectedBehavior}
            onChange={(e) => setNewCaseExpectedBehavior(e.target.value)}
            placeholder="What should happen when test is executed?"
          />
          <TextField
            margin="dense"
            ...
            multiline
            rows={2}
            value={newCaseActualBehavior}
            onChange={(e) => setNewCaseActualBehavior(e.target.value)}
            placeholder="What actually happened? (for bug tracking)"
          />
          ...
          <FormControl fullWidth margin="dense">
            ...
            <Select
              multiple
              value={newCaseRequirementIds}
              label="Requirements"
              onChange={(e) => setNewCaseRequirementIds(e.target.value as string[])}
              renderValue={(selected) => (
                <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                  {(selected as string[]).map((value) => {
                    const req = requirements.find(r => r.id === value);
                    return <Chip key={value} label={req?.title || value} size="small" />;
                  })}
                </Box>
              )}
            >
