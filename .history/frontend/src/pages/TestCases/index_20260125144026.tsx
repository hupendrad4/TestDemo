...
const TestCases: React.FC = () => {
  ...
  }, [currentProject?.id, location, loadTestCases]);

  useEffect(() => {
    filterTestCases();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [searchTerm, filterFormat, testCases, filterTestCases]);
  ...
  const filterTestCases = useCallback(() => {
  ...
  const handleCreateTestCase = async (testCase: any) => {
    try {
      if (!currentProject?.id) {
        ...
        setSnackbar({ open: true, message: 'Please select a project first', severity: 'error' });
        return;
      }
      ...
      const result = await testCaseService.createTestCase(currentProject.id, testCase);
      console.log('Test case created successfully:', result);
      
      setSnackbar({ open: true, message: 'Test case created successfully', severity: 'success' });
      setOpenDialog(false);
      loadTestCases();
    } catch (error: any) {
      console.error('Error creating test case:', error);
      const errorMessage = error?.response?.data?.message || error?.message || 'Failed to create test case';
    ...
    <Box>
      ...
      {!currentProject && (
        <Alert severity="warning" sx={{ mb: 3 }}>
          Please select a project from the dropdown in the header to view and create test cases.
        </Alert>
      )}
