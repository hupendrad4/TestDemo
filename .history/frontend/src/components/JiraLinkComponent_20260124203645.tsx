...
import jiraService, { JiraIssue, JiraLink } from '../../services/jira.service';
...
const JiraLinkComponent: React.FC<JiraLinkComponentProps> = ({
  ...
  const handleSearchIssues = async (query: string) => {
    ...
    try {
      ...
      const results = await jiraService.searchIssues(projectId, jql, 20);
  ...
  const handleLink = async () => {
    ...
    try {
      await jiraService.linkEntity(projectId, {
        ...
        entityType,
        entityId,
        jiraProjectId: selectedIssue.fields.project.id,
      });
  ...
  const handleUnlink = async (link: JiraLink) => {
    ...
    try {
      await jiraService.unlinkEntity(link.jiraIssueKey, entityType, entityId);
  ...
  const handleSync = async (link: JiraLink) => {
    try {
  ...
  const openInJira = (link: JiraLink) => {
    ...
    const integration = links[0];
    if (integration) {
      const jiraUrl = link.jiraProject?.id || 'https://jira.atlassian.net';
            ...
            <ListItem key={link.id} divider>
              ...
              <ListItemSecondaryAction>
                ...
                <Tooltip title="Sync to Jira">
                  <IconButton size="small" onClick={() => handleSync(link)}>
                    <Sync fontSize="small" />
                  </IconButton>
                </Tooltip>
